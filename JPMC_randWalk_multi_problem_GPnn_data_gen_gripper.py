...
"""
use pddl.py instance generation to get action instances, and then check if the state has already been seen.
From that state, generate a problem file, solve and generate data like before.
do not random step from the middle steps in the plan. or if you do, treat it as a separate point.
WAIT random steps from the middle maybe better. BUT do it carefully, at each point. Take a fixed number of random steps, and then solve.

See pyperplan code for state to pddl problem file generation to use with the existing code to produce solutions.
IT IS WORTH CLEANING up and organizing this code to make it faster.
...
for problem_config in all_configs:
    ...
    def replace_init_state_problem(problem_file_loc,curr_state,dest_problem_file_loc = "new_problem.pddl"):
        ...
        """
        problem_file_lines = []
        with open(problem_file_loc,"r") as src:
            old_lines = src.readlines()
        #---
        seen_init = False
        for single_line in old_lines:
            if "(:goal" in single_line:
                seen_init = False
            if "(:init" in single_line:
