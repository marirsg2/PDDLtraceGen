...
class Domain_manipulator:
    ...
    def parse_domain_file(self,domain_file_loc):
        ...
        predicates_start_token  = ":predicates"
        ...
        with open(domain_file_loc,"r") as domain_file:
            for line in domain_file:
                ...
                elif predicates_start_token in line:
                    parsing_state = self.Action_parsing_state.in_predicates
                        ...
                        if "(" in single_proposition:
                            ...
                            if len(proposition_parts_list) < 3 or len(proposition_parts_list) > 3: # len is 2 or 1
                                if negated_prop:
                                    ...
                                    proposition_parts_list.append("true")
    ...
    def get_all_parameter_groundings(self,action_name, problem_parser_obj):
        ...
        for single_type in parameter_types:
            if single_type != "false" and single_type != "true":
                try:
                    parameter_possible_values.append(problem_parser_obj.obj_dict[single_type])
                except:
                    #also check if there are lower types that can fill the slot
                    lower_types = self.to_lower_type_dict[single_type]
                    parameter_possible_values.append([])
                    for single_lower_type in lower_types:
                        parameter_possible_values[-1] = parameter_possible_values[-1] + problem_parser_obj.obj_dict[single_lower_type]
            else:
                parameter_possible_values.append([single_type])
        ...
        while True:
            ...
            if passed_static_precond:
                if len(set(current_var_instantitations)) != len(current_var_instantitations):
                    Warning("Dropping some cases as the same grounding is being used for two variables")
                else:
